...
int HP_CreateFile(char *fileName){
    ...
    if ((code = BF_CreateFile(fileName)) != BF_OK) // Create File
        return -1;
    if ((code = BF_OpenFile(fileName, &fd)) != BF_OK) // Load File
        return -1;
...
BF_ErrorCode HP_InsertEntry(int file_desc, HP_info* hp_info, Record record){
    BF_Block *block;
    BF_Block_Init(&block);
    int current_block;
    BF_ErrorCode code;
    ...
    if (current_block == 0){ // Edge case: No Blocks for records have been initialized
